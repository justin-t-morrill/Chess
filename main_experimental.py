...
def negaMax(board, alpha, beta, depth, tt, endTime):
    ...
    #Finds the hash move
    if board.state_stack[-1].zobrist in tt.keys():
        hash_move = tt[board.state_stack[-1].zobrist].hashMove
    else:
        hash_move = None

    for move in orderedMoves:
        #Searches next move with one less depth (0.5 less depth for the hash move)
        board.makeMove(move)
        if move == hash_move:
            invScore = negaMax(board, -beta, -alpha, depth - 0.5, tt, endTime)
        else:
        ...
        while aspirationalRepeat == True:
            ...
            for move in orderedMoves:
                ...
                if move != pv_move:
                    ...
                    #Looks one half depth further for the principal variation move
                    invScore = negaMax(board, -beta, -alpha, depth - 0.5, tt, endTime)
...
def printLine(board, move, tt, maxDepth):
    line = ""
...
def sortMoves(board, moves, tt):
    ...
    if board.state_stack[-1].zobrist in tt.keys():
        ...
        if possibleHashMove in moves:
            hashMove = possibleHashMove
            moves.remove(hashMove)

    sortedMoves = mvv_lva(board, moves)
    if hashMove != None:
        sortedMoves.insert(0, hashMove)
    return sortedMoves
...
if __name__ == "__main__":
    import main
    openings = []
    with open("8mvs_+90_+99.epd") as file:
        for line in file:
            openings.append(line)

    games = []
    for i in range(100):
        games.append(GameTest(rootNegaMax, main.rootNegaMax, 10, 1, openings=openings))
        print(games)
